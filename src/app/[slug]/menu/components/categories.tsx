...
const RestaurantCategories = ({restaurant}: RestaurantCategoriesProps) => {
    ...
    return (  <div className="relative z-50 mt-[-1.5rem] rounded-t-3xl  bg-white">
    <div className="p-5">
        ...
        <ScrollArea className="w-full ">
            <div className="flex w-max space-x-4 p-4 pt-0">
                {restaurant.menuCategories.map(category=>(
                    <Button className="rounded-full" key={category.id} variant={getCategoryButtonVarient(category)} size="sm" onClick={()=>handleCategoryClick(category)}>
                        {category.name}
                    </Button>
                ))}
            </div>
